...
def plot_data():
    ...
    legend_labels = []  # List to hold legend labels
    ...
    for group, color, label in zip(folder_paths, colors, group_labels):
        if check_vars[group].get() == 1:  # Check if the line is toggled on
            ...
            ax_magnitude.plot(filtered_freqs, mag_values, marker='o', color=color, label=label)
            legend_labels.append(label)  # Add label to legend
    ...
    ax_magnitude.grid()
    ax_magnitude.legend(legend_labels)  # Add legend for magnitude plot
    ...
    legend_labels = []  # Reset for phase plot
    ...
    for group, color, label in zip(folder_paths, colors, group_labels):
        if check_vars[group].get() == 1:  # Check if the line is toggled on
            ...
            ax_phase.plot(filtered_freqs, ph_values, marker='o', color=color, label=label)
            legend_labels.append(label)  # Add label to legend

    ax_phase.axhline(0, color='grey', lw=0.5, ls='--')
    ax_phase.grid()
    ax_phase.legend(legend_labels)  # Add legend for phase plot
